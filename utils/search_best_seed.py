...
LR = 0.001
FP_DIM = 512
SEEDS = [42, 123, 456, 789, 2024, 3141, 8888, 9999, 1234, 5678]  # 10个种子
...
class HighPCERegressorV3(nn.Module):
    ...
    def forward(self, data):
        ...
        g = torch.cat([g, self.fp_encoder(fp)], dim=1)
...
def evaluate(model, loader, device):
    ...
    return (mean_absolute_error(targets, preds),
            float(np.sqrt(np.mean((targets - preds) ** 2))),
            r2_score(targets, preds))


def train_with_seed(graphs, train_idx, val_idx, test_idx, seed, device):
    ...
    patience_counter = 0
    save_path = f'seed_{seed}_v3.pth'
...
def main():
    ...
    print(f"使用设备: {device}")
    print(f"将测试 {len(SEEDS)} 个随机种子\n")

    graphs, pce_values = load_data()
    print(f"数据加载完成: {len(graphs)} 个图\n")

    # 固定数据划分（所有种子使用相同的 train/val/test 划分）
    ...
    for i, seed in enumerate(SEEDS, 1):
        print(f"\n[{i}/{len(SEEDS)}] 训练 seed={seed}...")
        result = train_with_seed(graphs, train_idx, val_idx, test_idx, seed, device)
        results.append(result)
        print(f"  测试结果: MAE={result['test_mae']:.4f}, RMSE={result['test_rmse']:.4f}, R2={result['test_r2']:.4f}")
        print(f"  验证最佳: MAE={result['best_val_mae']:.4f}, 训练轮数: {result['epochs']}")
    ...
    for i, r in enumerate(results_sorted, 1):
        print(f"{i}. seed={r['seed']:5d} | MAE={r['test_mae']:.4f} | RMSE={r['test_rmse']:.4f} | R2={r['test_r2']:.4f}")

    best = results_sorted[0]
    print("\n" + "="*70)
    print(f"最优种子: {best['seed']}")
    print(f"  测试 MAE  = {best['test_mae']:.4f} %")
    print(f"  测试 RMSE = {best['test_rmse']:.4f} %")
    print(f"  测试 R2   = {best['test_r2']:.4f}")
    print(f"  模型保存: {best['save_path']}")
    print("="*70)

    # 保存结果到 CSV
    df_results = pd.DataFrame(results_sorted)
    df_results.to_csv('seed_search_results.csv', index=False)
    print(f"\n结果已保存至: seed_search_results.csv")
