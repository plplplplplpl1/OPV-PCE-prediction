"""
P1-5: GNN 超参数搜索扩展到 200 次 Optuna
复用原 train_gnn_hparam_search.py 的模型和数据逻辑，仅修改 trial 数
结果保存后与原 60 次结果对比，验证搜索是否饱和
"""
...
EPOCHS = 100
PATIENCE = 12
N_TRIALS = 200  # Expanded from 60 to 200
SEED = 9999
...
def main():
    ...
    print(f"Trials: {N_TRIALS} (previous: 60)")
    ...
    with open('external_results/gnn_hparam_search_200.json', 'w') as f:
    ...
    for t in study.trials:
        if t.value is not None:
            all_trials.append({'number': t.number, 'r2': t.value, 'params': t.params,
                               'state': str(t.state)})
    with open('external_results/gnn_hparam_search_200_all.json', 'w') as f:
        json.dump(all_trials, f, indent=2)

    # Importance analysis
    from optuna.importance import get_param_importances
    imp = get_param_importances(study)
    print("\nParameter importance (200 trials):")
    for k, v in sorted(imp.items(), key=lambda x: -x[1]):
        print(f"  {k}: {v:.4f}")

    # Compare with original 60-trial results
    try:
        with open('external_results/gnn_hparam_search.json') as f:
            old = json.load(f)
        print(f"\n{'='*50}")
        print(f"Comparison: 60 trials vs 200 trials")
        print(f"{'='*50}")
        print(f"  60 trials best R²: {old['best_r2']:.4f}")
        print(f"  200 trials best R²: {study.best_trial.value:.4f}")
        print(f"  ΔR² = {study.best_trial.value - old['best_r2']:.4f}")

        # Check if same params
        old_params = old['best_params']
        new_params = study.best_trial.params
        for k in sorted(set(list(old_params.keys()) + list(new_params.keys()))):
