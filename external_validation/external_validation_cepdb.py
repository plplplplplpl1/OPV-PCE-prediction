"""
CEPDB 大规模验证
在哈佛 CEPDB 25k 计算分子上比较 XGBoost vs GNN，测试不同数据规模下的表现
"""
import os, sys, json
import numpy as np
import pandas as pd
...
# ========== 加载 CEPDB ==========
print("="*50)
print("CEPDB Large-Scale Validation")
print("="*50)

df = pd.read_csv('/tmp/CEPDB_25000.csv')
# Filter to reasonable PCE range (>0 to avoid zeros)
df = df[df['pce'] > 0.01].copy().reset_index(drop=True)
# Use high-PCE subset (>3%) to match our paper's task
df_high = df[df['pce'] > 3.0].copy().reset_index(drop=True)
print(f"CEPDB high-PCE samples (PCE>3%): {len(df_high)}")

# Convert to graphs
graphs, pce_vals, failed = [], [], []
for i, (_, row) in enumerate(df_high.iterrows()):
    if i >= 2000:  # Cap at 2000 for computational feasibility
        break
...
print(f"Graph conversion: {len(graphs)} success, {len(failed)} failed (capped at 2000)")

# Subsampling for learning curve: train at different sizes
train_sizes = [100, 250, 500, 1000, 1500]
xgb_results_all = {}
gnn_results_all = {}
...
for n_train in train_sizes:
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(graphs)):
        train_smiles = [df_high.iloc[i]['SMILES_str'] for i in train_idx[:n_train]]
        train_pce = [df_high.iloc[i]['pce'] for i in train_idx[:n_train]]
        test_smiles = [df_high.iloc[i]['SMILES_str'] for i in test_idx]
    ...
    xgb_results_all[str(n_train)] = {'r2_mean': float(np.mean(xgb_r2)), 'r2_std': float(np.std(xgb_r2))}
    print(f"  XGBoost: R²={np.mean(xgb_r2):.4f}±{np.std(xgb_r2):.4f}")

# Save XGBoost results
with open('external_results/cepdb_xgb.json', 'w') as f:
    json.dump(xgb_results_all, f, indent=2)
print("\nXGBoost CEPDB results saved.")

# GNN for key training sizes only (most expensive)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Device: {device}")

for n_train in [250, 1000, 1500]:
    print(f"\n=== GNN Training size: {n_train} ===")
    gnn_r2 = []
    for fold in range(3):  # Only 3 folds for GNN (time constraints)
        set_seed(SEED + fold)
        ...
        for epoch in range(1, 101):
            ...
            if epoch % 25 == 0:
                ...
                print(f"  n={n_train} fold={fold} epoch={epoch} val_MAE={val_mae:.4f} val_R²={val_r2:.4f} patience={patience_counter}")
    ...
    if gnn_r2:
        gnn_results_all[str(n_train)] = {'r2_mean': float(np.mean(gnn_r2)), 'r2_std': float(np.std(gnn_r2))}
        print(f"  GNN n={n_train}: R²={np.mean(gnn_r2):.4f}±{np.std(gnn_r2):.4f}")

with open('external_results/cepdb_gnn.json', 'w') as f:
    json.dump(gnn_results_all, f, indent=2)
print("\nGNN CEPDB results saved.")

# Summary
print("\n" + "="*50)
print("CEPDB Validation Summary")
print("="*50)
for n in train_sizes:
    xgb = xgb_results_all.get(str(n), {})
    gnn = gnn_results_all.get(str(n), {})
    ...
    print(f"  n={n:5d}: XGBoost R²={xgb_str} | GNN R²={gnn_str}")
